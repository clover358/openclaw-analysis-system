...
# 报告输出目录：项目根 data/processed/（可被外部覆盖）
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "data" / "processed"

# ── 默认章节结构（与 Analyst 输出对齐：7 段式 OpenRouter 厂商分析）──────────
DEFAULT_REPORT_SECTIONS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("一、执行摘要",                 ("执行摘要", "摘要", "overview", "executive summary")),
    ("二、OpenAI 在 OpenRouter 的表现", ("openai 在 openrouter 的表现", "openai 表现", "主体表现", "openai")),
    ("三、竞品厂商格局",             ("竞品厂商格局", "竞品", "厂商格局", "竞争态势", "竞品分析")),
    ("四、应用与生态趋势",           ("应用与生态趋势", "应用生态", "生态趋势", "应用层趋势", "top apps")),
    ("五、行业宏观趋势",             ("行业宏观趋势", "行业趋势", "行业研判", "市场趋势", "ai index")),
    ("六、战略建议",                 ("战略建议", "策略建议", "行动建议", "建议")),
    ("七、风险提示",                 ("风险提示", "风险", "风险因素")),
)

DEFAULT_REPORT_TITLE_TEMPLATE = "OpenRouter LLM 厂商商业格局分析报告（{period}）"
...
def _base_style(ax, title: str, x_label: str = "", y_label: str = "") -> None:
    ax.set_facecolor(_BG)
    ax.figure.set_facecolor(_BG)
    ax.set_title(title, color=_TEXT, fontsize=13, fontweight="bold", pad=14)
    if x_label:
        ax.set_xlabel(x_label, color=_TEXT, fontsize=10)
    if y_label:
        ax.set_ylabel(y_label, color=_TEXT, fontsize=10)
...
def _bar_labels(ax, bars, values: list[float]) -> None:
    ...
    try:
        if ctype == "bar":
            ...
            wedges, texts, autotexts = ax.pie(
                ...
                pctdistance=0.82)
            for t in texts:
                t.set_color(_TEXT); t.set_fontsize(9.5)
            for at in autotexts:
...
# ════════════════════════════════════════════════════════════════════════════════
# ⑥ Markdown → PDF 渲染（两条降级路径：reportlab → fpdf2）
# ════════════════════════════════════════════════════════════════════════════════

def _find_cjk_font_path() -> str | None:
    """在常见系统位置查找一个支持中文的 TTF/OTF 字体。"""
    candidates = [
...
def _render_pdf_with_reportlab(md_text: str, output_path: Path, base_dir: Path | None = None) -> Path:
    """使用 reportlab 把 Markdown 渲染为 PDF（标题 / 列表 / 表格 / 段落 / 图片）。

    Args:
        md_text: 完整 Markdown 文本。
        output_path: 输出 PDF 路径。
        base_dir: 用于解析图片相对路径的基准目录（一般是 md 文件所在目录）。
    """
    ...
    if font_path:
        try:
            ...
            print(f"  [WARN] 注册中文字体失败，将退回 Helvetica: {exc}")
    ...
    h1 = ParagraphStyle(
        ...
        fontName=font_name, fontSize=18, leading=24, spaceAfter=10,
    )
    h2 = ParagraphStyle(
        "H2", parent=base_styles["Heading2"],
        fontName=font_name, fontSize=14, leading=20, spaceBefore=10, spaceAfter=6,
    )
    h3 = ParagraphStyle(
        "H3", parent=base_styles["Heading3"],
        fontName=font_name, fontSize=12, leading=18, spaceBefore=6, spaceAfter=4,
    )
    ...
    while i < len(lines):
        ...
        if stripped.startswith("|") and i + 1 < len(lines) and re.match(r"^\|[\s\-:|]+\|\s*$", lines[i + 1].strip()):
            ...
            tbl.setStyle(TableStyle([
                ...
                ("FONTSIZE", (0, 0), (-1, -1), 9),
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#E5ECF3")),
                ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#1F3A57")),
        ...
        elif (img_match := re.match(r"^!\[([^\]]*)\]\(([^)]+)\)\s*$", stripped)):
            alt, src = img_match.group(1), img_match.group(2).strip()
            img_path = Path(src)
            if not img_path.is_absolute():
                img_path = (base_dir or Path.cwd()) / img_path
            if img_path.exists():
                try:
                    ...
                    if alt:
                        caption = ParagraphStyle(
                            "Caption", parent=body, alignment=1,  # center
                            fontSize=9, textColor=colors.HexColor("#6B7C90"),
                            spaceBefore=2, spaceAfter=8,
                        )
                        flowables.append(Paragraph(_inline(alt), caption))
                except Exception as exc:
                    flowables.append(Paragraph(f"[图片渲染失败 {src}: {exc}]", body))
            else:
                flowables.append(Paragraph(f"[图片缺失: {src}]", body))
        elif stripped.startswith(("- ", "* ")):
            flowables.append(Paragraph("• " + _inline(stripped[2:]), bullet))
        elif re.match(r"^\d+\.\s", stripped):
            flowables.append(Paragraph(_inline(stripped), bullet))
        elif stripped.startswith("> "):
            flowables.append(Paragraph("<i>" + _inline(stripped[2:]) + "</i>", body))
        elif stripped.startswith("---"):
            flowables.append(Spacer(1, 0.3 * cm))
        else:
...
def _render_pdf_with_fpdf(md_text: str, output_path: Path) -> Path:
    """fpdf2 备用渲染：原样按行输出，不解析 markdown。"""
...
class GeneratorAgent:
    """通用商业分析报告生成智能体（DeepSeek 润色 + 智能图表）。"""
    ...
    def __init__(
        ...
        cfg_title = (report_title or report_cfg.get("title") or "").strip()
        self.report_title = cfg_title or self._default_dynamic_title()
        self.report_sections = report_sections or DEFAULT_REPORT_SECTIONS

        self.default_output = PROJECT_ROOT / report_cfg.get(
            "default_output",
            "data/processed/openrouter_llm_market_report.md",
        )
        ...
        # ── DeepSeek 客户端：优先使用入参，其次从 config/env 解析 ───────────
        ...
        if enable_polish or enable_chart:
            ...
            try:
                self._client = DeepSeekClient(api_key=resolved_key or DEEPSEEK_API_KEY)
    ...
    @staticmethod
    def _default_dynamic_title() -> str:
        """基于当前北京时间生成默认报告标题，避免硬编码年份。"""
        tz_cn = timezone(timedelta(hours=8))
        period = datetime.now(tz_cn).strftime("%Y-%m")
        return DEFAULT_REPORT_TITLE_TEMPLATE.format(period=period)
    ...
    def _resolve_api_key(self) -> str:
        """从 config.openai.api_key / 环境变量 OPENAI_API_KEY 解析 DeepSeek Key。"""
        import os
        openai_cfg = self.config.get("openai") or {}
        key = (openai_cfg.get("api_key") or "").strip()
        if not key:
    ...
    def generate_markdown_report(
        ...
        header = (
            ...
            f"| 项目 | 内容 |\n"
            f"| --- | --- |\n"
            f"| 报告类型 | OpenRouter LLM 厂商商业格局分析 |\n"
            f"| 主体定义 | 主产品 = OpenAI 旗下所有模型；竞品 = 其余厂商 |\n"
            f"| 生成时间 | {generated_at} |\n"
    ...
    @staticmethod
    def export_markdown_to_pdf(md_path: str | Path, pdf_path: str | Path | None = None) -> Path | None:
        """把 Markdown 报告渲染为 PDF。

        优先 reportlab（纯 Python，免外部依赖）；reportlab 不可用时尝试 fpdf2。
        两者都缺则打印警告并返回 None。
        """
        md_p = Path(md_path)
        if not md_p.exists():
            raise FileNotFoundError(f"Markdown 文件不存在: {md_p}")
        out_p = Path(pdf_path) if pdf_path else md_p.with_suffix(".pdf")
        ...
        # 尝试 reportlab
        try:
            return _render_pdf_with_reportlab(text, out_p, base_dir=md_p.parent)
        except ImportError:
            pass
        except Exception as exc:
            print(f"  [WARN] reportlab 渲染失败，尝试备用方案: {exc}")

        # 备用：fpdf2（不支持 markdown 语法，按行原样输出）
        try:
            return _render_pdf_with_fpdf(text, out_p)
        except ImportError:
            pass
        except Exception as exc:
            print(f"  [WARN] fpdf2 渲染失败: {exc}")

        print(
            "  [WARN] 未安装 reportlab 或 fpdf2，跳过 PDF 导出。"
    ...
    def run(self, analysis_text: str, report_filename: str | None = None) -> str:
        return self.generate_markdown_report(analysis_text, report_filename=report_filename)

    def save(self, report_content: str, output_path: str | Path) -> Path:
        """保存 Markdown 报告，并尝试同步导出同名 PDF（失败不影响主流程）。"""
        md_path = self.save_report(report_content, output_path)
        try:
            pdf_path = self.export_markdown_to_pdf(md_path)
            if pdf_path:
                print(f"[Generator] PDF 已导出 → {pdf_path}")
        except Exception as exc:
            print(f"[Generator] PDF 导出失败（已保留 Markdown）: {exc}")
        return md_path
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input",      "-i", required=True, help="输入分析文本路径")
    parser.add_argument("--output",     "-o", default=None,  help="输出报告路径")
    parser.add_argument("--output-dir", "-d", default=None,  help="报告与图表的输出目录")
    parser.add_argument("--api-key",    "-k", default=None,  help="DeepSeek API Key")
