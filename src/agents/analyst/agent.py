...
class AnalystAgent:
    ...
    def _get_retrieval_queries(self) -> dict[str, str]:
        ...
        return {
            "sales": retrieval_cfg.get("sales_query", "API token usage market share DeepSeek OpenAI"),
            "industry": retrieval_cfg.get("industry_query", "LLM API industry trend context length"),
            "competitor": retrieval_cfg.get("competitor_query", "competitor API Anthropic Google Gemini Claude GPT"),
        }
    ...
    def _build_prompts(
        ...
        system_prompt = (
            "你是一名资深 AI 平台与 API 竞品分析师，擅长整合多源 API 调用数据（市场份额、"
            "模型排行、上下文长度分布、综合榜单）撰写结构化、可落地的竞品分析报告。\n"
            "请严格基于提供的检索上下文作答，若某维度证据不足请明确指出数据缺口，"
            "禁止编造未出现的具体数字或事实。\n"
            "分析对象为主流大模型 API 提供商（如 DeepSeek、OpenAI、Anthropic、Google、Meta 等）。\n"
            "输出须包含以下 Markdown 章节：\n"
            "## 执行摘要\n"
            "## 主流 API 平台使用表现\n"
            "## 大模型 API 行业趋势研判\n"
        ...
        user_prompt = (
            ...
            f"## 检索上下文（RAG）\n{context_block}\n"
            "## 任务\n"
            "请综合以上三类证据，完成一份大模型 API 竞品深度分析报告。"
        )
