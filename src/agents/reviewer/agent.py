...
class ReviewerAgent:
    ...
    def _build_audit_messages(report_content: str, raw_data_summary: str) -> list:
        system_prompt = (
            ...
            "【数据结论数值支撑强约束】\n"
            "凡报告中出现“表现如下”“排名中”“榜单中”“检索到”“位列”“市场份额”“调用量”等数据性结论，"
            "必须同时出现 rank/排名、tokens/调用量、份额/share、change/变化率、count/请求数中的至少一个数值字段。"
            "若只罗列模型名称或厂商名称、没有任何数值支撑，必须判定 is_passed=false，并要求删除该段或补充原始数据中的数值。\n"
            "报告不得使用 Markdown 连字符列表（- item）；若出现模型名称列表，必须改为编号列表且每项带数值。\n\n"
            "【图片引用保留强约束】\n"
